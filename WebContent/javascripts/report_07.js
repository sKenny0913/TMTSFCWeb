$(document).ready(function () {
    ...
    $("#btnSearch").click(function () {
        //        $("#idTable").hide();
        var url = serverAddress + "Report_07";
        var sWO = $(this).closest("form").find("input[name='WO']").val();
        var sStation = $(this).closest("form").find("input[name='Station']").val();
        var sSKU = $(this).closest("form").find("input[name='SKU']").val();
        var sSN = $(this).closest("form").find("input[name='SN']").val();
        var urlFinal = url + '?WO=' + sWO + '&STATION_CODE=' + sStation + '&SN_Type=' + sSKU + '&SN=' + sSN;
...
$(document).on('click', '.w3-medium', function () {
    var sWO = $(this).find("td:eq(2)").text().trim();
    var sStation = $(this).find("td:eq(3)").text().trim();
    var sSKU = $(this).find("td:eq(1)").text().trim();
    var sSN = $(this).find("td:eq(0)").text().trim();
    $("input[name='WO']").val(sWO);
    $("select[id='idStationList']").val(sStation);
    $("input[name='SKU']").val(sSKU);
